<div>
    <section class="mt-3">
        ...
        <div class="mb-6 bg-white rounded-lg shadow-lg">
            <div class="flex items-center justify-end px-6 py-2">
                ...
                <div class="grid grid-cols-2 text-gray-500 border border-gray-200 divide-x divide-gray-200 md:block">
                    <i class="fa fa-border-all p-2 cursor-pointer {{ $view == 'grid' ? 'text-orange-500' : ''}}" wire:click="$set('view', 'grid')"></i>
                    <i class="fas fa-th-list p-2 cursor-pointer {{ $view == 'list' ? 'text-orange-500' : ''}}" wire:click="$set('view', 'list')"></i>
                </div>
            </div>
        </div>


        <h1 class="mb-6 text-3xl text-center text-gray-600">Ultimas Productos Registrados</h1>
        ...
        @if ($view == 'grid')
            ...
            <div class="grid px-4 mx-auto mt-6 text-center w-72 max-w-7xl sm:px-6 lg:px-8 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-x-6 gap-y-8">
                <a href="{{ route('listproducts') }}" class="btn btn-danger"> Ver todo los Productos</a>
            </div>
        @else
                ...
                <article class="md:flex">
                         ...
                         @if($product->photos->count() )
                            @foreach ( $product->photos->take(1) as $photo)
                                <a href="{{ route('verproducto', $product )}}">
                                    <img class="object-cover object-center w-full h-48 md:w-56" src="{{ Storage::disk("s3")->url($photo->url) }}" alt="{{ $product->name }}">
                                </a>
                            @endforeach
                        @else
                            <a href="{{ route('verproducto', $product )}}">
                    ...
                    <div class="flex flex-col flex-1 px-6 py-4">
                        <div class="justify-between lg:flex">
                            <div>
                                <h1 class="text-lg font-semibold text-gray-700 capitalize">{{ $product->name }}</h1>
                                ...
                                <p class="font-bold text-gray-700">{{ Str::limit($product->shortdescription, 100) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3 w-60">
                                <a href="{{ route('showempresa', $product->user )}}" class="right-0 mt-2 btn btn-primary btn-block">
                                    Ver Proveedor
                                </a>
                            </div>
                        </div>
